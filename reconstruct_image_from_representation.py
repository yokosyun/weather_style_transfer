    ...
    def tuning_step(optimizing_img):
        ...
        if should_reconstruct_content:
            loss = torch.nn.MSELoss(reduction="mean")(
                target_representation, current_representation
            )
        else:
...
def reconstruct_image_from_representation(config):
    ...
    white_noise_img = np.random.uniform(-90.0, 90.0, img.shape).astype(np.float32)
    ...
    if should_reconstruct_content:
        ...
        if should_visualize_representation:
            ...
            for i in range(num_of_gram_matrices):
                Gram_matrix = (
                    target_style_representation[i].squeeze(axis=0).to("cpu").numpy()
                )
    ...
    if config["optimizer"] == "adam":
        ...
        target_representation = (
            ...
            if should_reconstruct_content
            else target_style_representation
        )
        ...
        def closure():
            ...
            if should_reconstruct_content:
                ...
                for gram_gt, gram_hat in zip(
                    ...
                    loss += (1 / len(target_style_representation)) * torch.nn.MSELoss(
                        reduction="sum"
                    )(gram_gt[0], gram_hat[0])
            loss.backward()
            with torch.no_grad():
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--should_reconstruct_content",
        type=bool,
        help="pick between content or style image reconstruction",
        default=True,
    )
    parser.add_argument(
        "--should_visualize_representation",
        type=bool,
        help="visualize feature maps or Gram matrices",
        default=False,
    )

    parser.add_argument(
        "--content_img_name", type=str, help="content image name", default="lion.jpg"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--reconstruct_script",
        type=str,
        help="dummy param - used in saving func",
        default=True,
    )
